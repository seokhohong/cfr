...
class KuhnCFRTrainer():
    ...
    def debug_one(self):
        ...
        history = ''
        reach_probabilities = np.ones(self.num_players)
        self.cfr([0, 1, 2], history, reach_probabilities, len(history) % len(cards))
        history = 'BC'
        eval = self.cfr(cards, history, reach_probabilities, len(history) % len(cards))
        print(eval)
    ...
    def debug_all(self):
        ...
        history = ''
        reach_probabilities = np.ones(self.num_players)
        val = np.zeros(self.num_players)
        for cards in itertools.permutations(all_cards, 3):
            eval = self.cfr(cards, history, reach_probabilities, len(history) % len(cards))
            print(cards, eval)
            val += eval
        print(val)
